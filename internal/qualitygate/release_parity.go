...
func releaseParity(ctx context.Context, root string) error {
	parent, err := os.MkdirTemp("", "starter-oauth2client-release-parity-*")
	if err != nil {
	...
	for _, outputDir := range retainedOutputs {
		if commandErr := command(
			...
			"go",
			"run",
			"./cmd/starter-oauth2client-release",
			"-rehearsal",
			"-version="+rehearsalVersion,
			"-output="+outputDir,
		); commandErr != nil {
...
func validateReleaseParity(
	...
	base := "starter-oauth2client_" + strings.TrimPrefix(rehearsalVersion, "v")
	...
	prefix := "starter-oauth2client_" + strings.TrimPrefix(rehearsalVersion, "v") + "/"
...
func readParityArchive(filename string, expectedPrefix string) (parityArchive, error) {
	...
	if compressed.Len() != 0 {
		return parityArchive{}, errors.New("compressed archive has trailing data or additional gzip members")
	}
	return result, nil
}
...
func readBoundedParityArchive(filename string) (_ []byte, resultErr error) {
	...
	if err != nil {
		return nil, fmt.Errorf("resolve compressed archive: %w", err)
	}
	root, err := os.OpenRoot(filepath.Dir(absolute))
	if err != nil {
		return nil, fmt.Errorf("open compressed archive root: %w", err)
	}
	defer func() { resultErr = errors.Join(resultErr, root.Close()) }()
	name := filepath.Base(absolute)
	before, err := root.Lstat(name)
	if err != nil {
		return nil, fmt.Errorf("inspect compressed archive: %w", err)
	}
	if !before.Mode().IsRegular() || before.Size() < 0 || before.Size() > maximumParityArchiveSize {
		return nil, fmt.Errorf("compressed archive must be a regular file bounded to %d bytes", maximumParityArchiveSize)
	}
	file, err := root.Open(name)
	if err != nil {
		return nil, fmt.Errorf("open compressed archive: %w", err)
	}
	defer func() { resultErr = errors.Join(resultErr, file.Close()) }()
	after, err := file.Stat()
	if err != nil {
		return nil, fmt.Errorf("inspect open compressed archive: %w", err)
	}
	if !after.Mode().IsRegular() || after.Size() < 0 ||
		after.Size() > maximumParityArchiveSize || !os.SameFile(before, after) {
		return nil, errors.New("compressed archive changed while it was opened")
	}
...
func validateReleaseSBOMParity(centralContent, retainedContent []byte) error {
	...
	baseNamespace := "https://github.com/spice-framework/starter-oauth2client/releases/" +
...
func validateCentralSBOMProvenance(document releaseSBOM, baseNamespace string) error {
	if document.Name != "starter-oauth2client "+rehearsalVersion ||
...
func validateRetainedSBOMProvenance(document releaseSBOM, baseNamespace string) error {
	if document.Name != "Spice OAuth2 Client starter "+rehearsalVersion ||
		...
		return errors.New("retained release SBOM provenance does not match the OAuth2 Client builder")
	}
	return nil
}
...
func readReleaseArtifact(rootPath string, name string) (_ []byte, resultErr error) {
	...
	if !before.Mode().IsRegular() || before.Size() < 0 || before.Size() > maximumParityMetadata {
		return nil, fmt.Errorf(
			"release artifact %q must be a regular file bounded to %d bytes",
			name,
			maximumParityMetadata,
		)
	}
	file, err := root.Open(name)
	if err != nil {
	...
	after, err := file.Stat()
	if err != nil {
		return nil, fmt.Errorf("inspect open release artifact %q: %w", name, err)
	}
	if !after.Mode().IsRegular() || after.Size() < 0 ||
		after.Size() > maximumParityMetadata || !os.SameFile(before, after) {
		return nil, fmt.Errorf("release artifact %q changed while it was opened", name)
	}
	content, err := io.ReadAll(io.LimitReader(file, maximumParityMetadata+1))
	if err != nil {
		return nil, fmt.Errorf("read release artifact %q: %w", name, err)
	}
	if len(content) > maximumParityMetadata {
		return nil, fmt.Errorf("release artifact %q exceeds %d bytes", name, maximumParityMetadata)
	}
	return content, nil
}
