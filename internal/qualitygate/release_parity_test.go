...
func TestValidateReleaseArchiveParity(t *testing.T) {
	...
	writeParityTestArchive(t, central, "starter-oauth2client_0.0.0-rehearsal/", "same", "")
	writeParityTestArchive(t, retained, "starter-oauth2client_0.0.0-rehearsal/", "same", "")
	if err := validateReleaseArchiveParity(
		central,
		"starter-oauth2client_0.0.0-rehearsal/",
		retained,
		"starter-oauth2client_0.0.0-rehearsal/",
	); err != nil {
	...
	t.Run("rejects entry drift", func(t *testing.T) {
		...
		writeParityTestArchive(t, drifted, "starter-oauth2client_0.0.0-rehearsal/", "different", "")
	...
	t.Run("rejects unexpected retained root", func(t *testing.T) {
		...
		writeParityTestArchive(t, wrong, "starter-oauth2client-0.0.0-rehearsal/", "same", "")
		err := validateReleaseArchiveParity(
			central,
			"starter-oauth2client_0.0.0-rehearsal/",
			wrong,
	...
	t.Run("rejects hidden decompressed tail", func(t *testing.T) {
		...
		prefix := "starter-oauth2client_0.0.0-rehearsal/"
	...
	t.Run("rejects corrupt gzip checksum", func(t *testing.T) {
		t.Parallel()
		wrong := filepath.Join(t.TempDir(), "corrupt-crc.tar.gz")
		content := readParityTestFile(t, central)
		content[len(content)-8] ^= 0xff
		writeParityTestPath(t, wrong, content)
		if _, err := readParityArchive(wrong, "starter-oauth2client_0.0.0-rehearsal/"); err == nil {
			t.Fatal("readParityArchive(corrupt CRC) error = nil")
		}
	})

	t.Run("rejects truncated gzip trailer", func(t *testing.T) {
		t.Parallel()
		wrong := filepath.Join(t.TempDir(), "truncated.tar.gz")
		content := readParityTestFile(t, central)
		writeParityTestPath(t, wrong, content[:len(content)-4])
		if _, err := readParityArchive(wrong, "starter-oauth2client_0.0.0-rehearsal/"); err == nil {
			t.Fatal("readParityArchive(truncated trailer) error = nil")
		}
	})

	t.Run("rejects raw compressed tail", func(t *testing.T) {
		t.Parallel()
		wrong := filepath.Join(t.TempDir(), "raw-tail.tar.gz")
		content := append(readParityTestFile(t, central), []byte("hidden")...)
		writeParityTestPath(t, wrong, content)
		if _, err := readParityArchive(wrong, "starter-oauth2client_0.0.0-rehearsal/"); err == nil ||
			!strings.Contains(err.Error(), "trailing data") {
			t.Fatalf("readParityArchive(raw tail) error = %v", err)
		}
	})

	t.Run("rejects additional gzip member", func(t *testing.T) {
		t.Parallel()
		wrong := filepath.Join(t.TempDir(), "multiple-members.tar.gz")
		var additional bytes.Buffer
		writer := gzip.NewWriter(&additional)
		if _, err := writer.Write([]byte("hidden")); err != nil {
			t.Fatal(err)
		}
		if err := writer.Close(); err != nil {
			t.Fatal(err)
		}
		content := append(readParityTestFile(t, central), additional.Bytes()...)
		writeParityTestPath(t, wrong, content)
		if _, err := readParityArchive(wrong, "starter-oauth2client_0.0.0-rehearsal/"); err == nil ||
			!strings.Contains(err.Error(), "additional gzip members") {
			t.Fatalf("readParityArchive(additional member) error = %v", err)
		}
	})
}
...
func TestValidateReleaseSBOMParity(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "wrong OAuth2 Client provenance",
			mutate: func(_ *releaseSBOM, retained *releaseSBOM) {
				retained.CreationInfo.Creators[0] = "Organization: Unknown"
			},
			wantErr: "OAuth2 Client builder",
		},
		{
			name: "relationship drift",
			mutate: func(_ *releaseSBOM, retained *releaseSBOM) {
				retained.Relationships[0].RelatedSPDXElement = "SPDXRef-Package-wrong"
			},
			wantErr: "outside documented provenance",
		},
	}
...
func TestValidateReleaseParityRejectsSignaturesAndBadChecksums(t *testing.T) {
	...
	t.Run("archive bytes", func(t *testing.T) {
		t.Parallel()
		drifted := maps.Clone(retained)
		drifted[releaseArchiveName()] = sha256.Sum256([]byte("different archive"))
	...
	t.Run("noncanonical checksum spacing", func(t *testing.T) {
		t.Parallel()
		corruptRoot, corrupt := writeParityTestRelease(t, true)
		checksumPath := filepath.Join(corruptRoot, "checksums.txt")
		content := strings.ReplaceAll(string(readParityTestFile(t, checksumPath)), "  ", " ")
		writeParityTestPath(t, checksumPath, []byte(content))
		corrupt["checksums.txt"] = sha256.Sum256([]byte(content))
		err := validateReleaseParity(corruptRoot, corrupt, retainedRoot, retained)
...
func paritySBOMFixtures() (releaseSBOM, releaseSBOM) {
	...
	central := common
	central.Name = "starter-oauth2client " + rehearsalVersion
	...
	retained := common
	retained.Name = "Spice OAuth2 Client starter " + rehearsalVersion
	retained.DocumentNamespace = parityNamespace("", 'b')
	retained.CreationInfo.Creators = []string{
		"Organization: Spice Authors",
		"Tool: github.com/spice-framework/starter-oauth2client/cmd/starter-oauth2client-release",
	}
...
func releaseArchiveName() string {
	return "starter-oauth2client_" + strings.TrimPrefix(rehearsalVersion, "v") + "_source.tar.gz"
}

func releaseSBOMName() string {
	return "starter-oauth2client_" + strings.TrimPrefix(rehearsalVersion, "v") + "_sbom.spdx.json"
}

func parityNamespace(extra string, digit rune) string {
	return "https://github.com/spice-framework/starter-oauth2client/releases/" + rehearsalVersion +
...
func writeParityTestFile(t *testing.T, root string, name string, content []byte) {
	t.Helper()
	writeParityTestPath(t, filepath.Join(root, name), content)
}

func writeParityTestPath(t *testing.T, filename string, content []byte) {
	t.Helper()
...
func readParityTestFile(t *testing.T, filename string) []byte {
	t.Helper()
